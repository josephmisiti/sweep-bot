...
git_image = modal.Image.debian_slim().apt_install("git").pip_install("GitPython", 'openai', "PyGithub")
...
def handle_ticket(request: dict):
    ...
    if "issue" in request and request["action"] == "opened":
        ...
        if repo_description is None:
            repo_description = ""
        return on_ticket(title, body, number, issue_url, repo_full_name, repo_description)
    return {"success": True}
